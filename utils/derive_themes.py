...
def derive_themes_from_keywords(df: pd.DataFrame, theme_map: dict) -> pd.DataFrame:
    ...
    # Normalize Unicode case variants in both the configured keywords and source
    # text. For example, Python's case-insensitive regex considers the long s
    # ("ſ") equivalent to "s", but returns the source spelling from findall().
    # Casefolding first ensures that a match such as "coaſt" can be looked up as
    # the configured keyword "coast".
    normalized_theme_map = {
        str(keyword).casefold(): theme for keyword, theme in theme_map.items()
    }
    ...
    regex_pattern = (
        r"\b("
        + "|".join(re.escape(keyword) for keyword in normalized_theme_map)
        + r")\b"
    )
    keyword_regex = re.compile(regex_pattern)
    ...
    def _find_themes_for_row(row):
        ...
        search_text = " ".join([title_text, keyword_text, subject_text]).casefold()
        
        if not search_text.strip():
            return None

        # Find all non-overlapping matches for the keywords in the search text
        found_keywords = set(keyword_regex.findall(search_text))
        ...
        matched_themes = {
            ...
            if (theme := normalized_theme_map.get(keyword)) is not None
        }
